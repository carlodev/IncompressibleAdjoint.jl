...
for CL_target in [0.45,0.55,0.65,0.7]
...
"""
Optimization function, it aims for a CL=CL_target.
...
function compute_CL(uh,ph,nΓ,dΓ,params)
    ...
    return 0.5*(CL-CL_target)^2, CL
end



βv= [Vector{Float64}(undef, Ndes) for _ = 1:30]
airfoil_nodes_top, airfoil_nodes_bottom,_,_ = get_airfoil_characteristics(model, params; tag="airfoil")

nodes_top = [Vector{Float64}(undef, length(airfoil_nodes_top)) for _ = 1:30] 
nodes_bottom = [Vector{Float64}(undef, length(airfoil_nodes_bottom)) for _ = 1:30] 
cp_top = [Vector{Float64}(undef, length(airfoil_nodes_top)) for _ = 1:30] 
cp_bottom = [Vector{Float64}(undef, length(airfoil_nodes_bottom)) for _ = 1:30] 
...
CL_history = [Vector{Float64}(undef, length(PH)) for _ = 1:30] 
...
merge!(params,Dict(:uh00=>uh,:ph00=>ph, :UH=>UH,:PH=>PH, :CL_history=>CL_history, :CL_target=>CL_target))
...
function optimization_loop(w::Vector,grad::Vector)
 @unpack des_points,Ndes,shift,AoA, obj_fun, iter, 
            mesh_ref,uh00,ph00,UH,PH,CL_target=params
    ...
    params[:d_boundary] = VectorValue(0.0, (CL_target-CL))
    ...
    println("CL = $CL")
    jldsave("results/NACA0012_1000_2p5/parameters_$(CL_target).jld2"; params)
    return J1ref
...
using JLD2
CLtarget = 0.55
params = load("results/NACA0012_1000_2p5/parameters_$CLtarget.jld2")["params"]
@unpack iter,dt,tf = params
ttime = collect(0.0:dt:tf)

plot(collect(1:1:iter) .-1,params[:CL][1:iter] ,linewidth=2.0,linecolor=:teal,label=false)
plot!(collect(1:1:iter) .-1, CLtarget.*ones(iter), linestyle=:dash,linecolor=:black, linewidth=1.2,label="target")
...
p_airfoil = update_CST_weights(params[:βv][iter],des_points)
plot(p_airfoil.apoints.xu,p_airfoil.apoints.yu,linewidth=2,linecolor=:black,label=false)
plot!(p_airfoil.apoints.xl,p_airfoil.apoints.yl,linewidth=2,linecolor=:black,label=false)
plot!(aspect_ratio = :equal)

savefig("airfoil$CLtarget.pdf")
