...
"""
NACA0012 - Reynolds 1000, AoA 4.0, CL adjoint optimization, aiming a targhet CL=0.45.
...
### Nt: when to start time-averaging
@unpack t0,dt,tf = params
ttime = collect(0.0:dt:tf)
Nt = Int64.(ceil(length(ttime)*0.25))

merge!(params,Dict(:uh00=>uh,:ph00=>ph, :UH=>UH,:PH=>PH, :CL_history=>CL_history, :Nt=>Nt))
...
function optimization_loop(w::Vector,grad::Vector)
 @unpack des_points,Ndes,shift,AoA, obj_fun, iter, 
            mesh_ref,uh00,ph00,UH,PH,Nt=params
    ...
    params[:d_boundary] = VectorValue(0.0, (0.55-CL))
    

    ### run a steady adjoint on the average field

    average_field!(uh,UH[end-Nt:end])
    average_field!(ph,PH[end-Nt:end])
    ...
    println("CL = $CL")
    jldsave("results/NACA0012_1000_2p5/parameters_newstart.jld2"; params)
    return J1ref
...
# Post processing
######################################################################
params = load("results/NACA0012_1000_4/parameters_newstart.jld2")["params"]
@unpack iter = params
...
plot!(collect(1:1:iter) .-1, 0.45.*ones(iter), linestyle=:dash,linecolor=:black, linewidth=1.2,label="target")
